...
#   dbg('fix(' + `filename` + ')\n')
    ...
    try:
#       dbg('os.rename(' + tempname + ', ' +filename + ')\n')
        os.rename(tempname, filename)
    except os.error, msg:
        err(filename + ': rename from ' + tempname + 'failed (' + str(msg) + ')\n')
        return 1
    # Return succes
    return 0
...
Identifier = '\(record\)?[a-zA-Z_][a-zA-Z0-9_]+'
#String = '"\([^\n\\"]\|\\\\.\)*"'
Char = '\'\([^\n\\\']\|\\\\.\)*\''
CommentStart = '(\*'
CommentEnd = '\*)'
...
OutsideComment = (Identifier, Number, Char, CommentStart, CommentStart2)
OutsideCommentPattern = '\(' + string.joinfields(OutsideComment, '\|') + '\)'
OutsideCommentProgram = regex.compile(OutsideCommentPattern)

InsideComment = (Identifier, Number, CommentEnd, CommentEnd2)
