...
def load_textures(md2):
    ...
    if int(md2.num_skins) > 0:
        ...
        #    imagecopy = file.read()
            file.close()
            skins[md2.skins[i].name] = imagecopy
...
def load_md2(md2_filename):
 ...
 #       o.write(Header)
  ...
  #  mesh = NMesh.New()
    ...
    mdl_component['Skins:sg'] = load_textures(md2) # Calls here to make the Skins Group.
 ...
 #   mesh.hasFaceUV(1)  #turn on face UV coordinates for this mesh
  ...
  #      face = NMesh.Face()
     ...
     #   face.v.append(mesh.verts[md2.faces[i].vertex_index[1]])
        ...
        if i < 4:
            print "line 489 vertex_index",md2.faces[i].vertex_index
...
ModelRoot = {}
# ModelRoot['Model:mr'] = load_md2(md2_filename)
# if ModelRoot['Model:mr'] is None:
