...
//
BOOL CWADFile::GetImageSize(CDirectoryFile *File, DWORD &dwPaletteSize, DWORD &dwPixelSize)
{
	if(File->GetID() == HLLIB_INVALID_ID)
	{
		dwPaletteSize = 0;
...
//
BOOL CWADFile::GetImageData(CDirectoryFile *File, DWORD &dwWidth, DWORD &dwHeight, LPBYTE lpPaletteData, LPBYTE lpPixelData)
{
	if(File->GetID() == HLLIB_INVALID_ID)
	{
		this->SetLastError("Invalid file ID.");
...
//
BOOL CWADFile::GetImageSize(CDirectoryFile *File, DWORD &dwSize)
{
	if(File->GetID() == HLLIB_INVALID_ID)
	{
		dwSize = 0;
...
//
BOOL CWADFile::GetImageData(CDirectoryFile *File, DWORD &dwWidth, DWORD &dwHeight, LPBYTE lpData)
{
	if(File->GetID() == HLLIB_INVALID_ID)
	{
		this->SetLastError("Invalid file ID.");
...
void CWADFile::GetLumpInfo(DWORD dwLumpIndex, DWORD &dwWidth, DWORD &dwHeight, DWORD &dwPaletteSize, LPBYTE &lpPalette, LPBYTE &lpPixels)
{
	...
	// Scan past data.
	lpMipPos += (dwWidth * dwHeight) + (dwWidth * dwHeight / 4) + (dwWidth * dwHeight / 16) + (dwWidth * dwHeight / 64);
