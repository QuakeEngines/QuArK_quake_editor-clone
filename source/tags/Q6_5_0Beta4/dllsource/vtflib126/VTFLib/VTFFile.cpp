/*
 * VTFLib
 * Copyright (C) 2005-2007 Neil Jedrzejewski & Ryan Gregg
...
//       tested with version 8.31.0225.1745, availible here:
...
vlBool CVTFFile::Create(vlUInt uiWidth, vlUInt uiHeight, vlUInt uiFrames, vlUInt uiFaces, vlUInt uiSlices, vlByte **lpImageDataRGBA8888, const SVTFCreateOptions &VTFCreateOptions)
{
	...
	if(VTFCreateOptions.uiVersion[0] != 7 || (VTFCreateOptions.uiVersion[1] != 0 && VTFCreateOptions.uiVersion[1] != 1 && VTFCreateOptions.uiVersion[1] != 2 && VTFCreateOptions.uiVersion[1] != 3))
	{
		LastError.SetFormatted("File version %u.%u does not match 7.0 or 7.1 or 7.2 or 7.3.", VTFCreateOptions.uiVersion[0], VTFCreateOptions.uiVersion[1]);
		return vlFalse;
	}
...
vlBool CVTFFile::Load(IO::Readers::IReader *Reader, vlBool bHeaderOnly)
{
	...
	try
	{
		...
		if(FileHeader.Version[0] != 7 || (FileHeader.Version[1] != 0 && FileHeader.Version[1] != 1 && FileHeader.Version[1] != 2 && FileHeader.Version[1] != 3))
		{
			LastError.SetFormatted("File version %u.%u does not match 7.0 or 7.1 or 7.2 or 7.3.", FileHeader.Version[0], FileHeader.Version[1]);
			throw 0;
		}
...
vlVoid CVTFFile::SetFlags(vlUInt uiFlags)
{
	...
	if(this->Header->Flags & TEXTUREFLAGS_NOCOMPRESS)
		uiFlags |= TEXTUREFLAGS_NOCOMPRESS;
	else
		uiFlags &= ~TEXTUREFLAGS_NOCOMPRESS;
