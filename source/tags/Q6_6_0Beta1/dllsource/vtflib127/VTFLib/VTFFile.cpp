...
vlBool CVTFFile::Create(vlUInt uiWidth, vlUInt uiHeight, vlUInt uiFrames, vlUInt uiFaces, vlUInt uiSlices, vlByte **lpImageDataRGBA8888, const SVTFCreateOptions &VTFCreateOptions)
{
	...
	if(VTFCreateOptions.uiVersion[0] != 7 || (VTFCreateOptions.uiVersion[1] < 0 || VTFCreateOptions.uiVersion[1] > 4))
	{
		LastError.SetFormatted("File version %u.%u does not match 7.0 to 7.4.", VTFCreateOptions.uiVersion[0], VTFCreateOptions.uiVersion[1]);
		return vlFalse;
	}
...
vlBool CVTFFile::Load(IO::Readers::IReader *Reader, vlBool bHeaderOnly)
{
	...
	try
	{
		...
		if(FileHeader.Version[0] != 7 || (FileHeader.Version[1] < 0 || FileHeader.Version[1] > 4))
		{
			LastError.SetFormatted("File version %u.%u does not match 7.0 to 7.4.", FileHeader.Version[0], FileHeader.Version[1]);
			throw 0;
		}
