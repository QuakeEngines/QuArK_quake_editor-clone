...
CVMTGroupNode::CVMTGroupNode(const CVMTGroupNode &GroupNode) : CVMTNode(GroupNode.GetName())
{
	for(CVMTNodeList::const_iterator it = GroupNode.VMTNodeList->begin(); it != GroupNode.VMTNodeList->end(); ++it)
	{
		...
		switch(VMTNode->GetType())
		{
		case NODE_TYPE_GROUP:
			this->VMTNodeList->push_back(new CVMTGroupNode(*static_cast<CVMTGroupNode *>(VMTNode)));
			break;
		case NODE_TYPE_STRING:
			this->VMTNodeList->push_back(new CVMTStringNode(*static_cast<CVMTStringNode *>(VMTNode)));
			break;
		case NODE_TYPE_INTEGER:
			this->VMTNodeList->push_back(new CVMTIntegerNode(*static_cast<CVMTIntegerNode *>(VMTNode)));
			break;
		case NODE_TYPE_SINGLE:
			this->VMTNodeList->push_back(new CVMTSingleNode(*static_cast<CVMTSingleNode *>(VMTNode)));
			break;
		}
	}
}
...
vlVoid CVMTGroupNode::AddNode(CVMTNode *VMTNode)
{
