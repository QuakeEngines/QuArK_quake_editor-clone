...
#define HL_VERSION_NUMBER	203
#define HL_VERSION_STRING	"2.0.3"
...
typedef enum
{
	...
	HL_VBSP_ITEM_COUNT,
	HL_VBSP_ZIP_PACKAGE_DISK = HL_VBSP_ITEM_COUNT,
	HL_VBSP_ZIP_PACKAGE_COMMENT,
	HL_VBSP_ZIP_PACKAGE_COUNT,
	HL_VBSP_ZIP_ITEM_CREATE_VERSION = 0,
...
typedef hlUInt (*PWriteProc)  (hlVoid *, hlUInt, hlVoid *);
typedef hlUInt (*PSeekProc) (hlLong, HLSeekMode, hlVoid *);
typedef hlUInt (*PTellProc) (hlVoid *);
typedef hlUInt (*PSizeProc) (hlVoid *);

typedef hlVoid (*PExtractItemStartProc) (HLDirectoryItem *pItem);
typedef hlVoid (*PExtractItemEndProc) (HLDirectoryItem *pItem, hlBool bSuccess);
typedef hlVoid (*PExtractFileProgressProc) (HLDirectoryItem *pFile, hlUInt uiBytesExtracted, hlUInt uiBytesTotal, hlBool *pCancel);
typedef hlVoid (*PValidateFileProgressProc) (HLDirectoryItem *pFile, hlUInt uiBytesValidated, hlUInt uiBytesTotal, hlBool *pCancel);
typedef hlVoid (*PDefragmentProgressProc) (HLDirectoryItem *pFile, hlUInt uiFilesDefragmented, hlUInt uiFilesTotal, hlUInt uiBytesDefragmented, hlUInt uiBytesTotal, hlBool *pCancel);
...
HLLIB_API hlUInt hlStreamSeek(HLStream *pStream, hlLong iOffset, HLSeekMode eSeekMode);
...
HLLIB_API hlUInt hlStreamWrite(HLStream *pStream, hlVoid *lpData, hlUInt uiBytes);
...
HLLIB_API hlUInt hlPackageGetItemAttributeCount(const HLDirectoryItem *pItem);
...
namespace HLLib
{
	...
	public:
		...
		hlBool CreateStream(Streams::IStream *&pStream);
		hlVoid ReleaseStream(Streams::IStream *pStream);
	...
	namespace Streams
	{
		...
		public:
			...
			virtual hlUInt GetStreamSize() const = 0;
			virtual hlUInt GetStreamPointer() const = 0;

			virtual hlUInt Seek(hlLong iOffset, HLSeekMode eSeekMode) = 0;

			virtual hlBool Read(hlChar &cChar) = 0;
			virtual hlUInt Read(hlVoid *lpData, hlUInt uiBytes) = 0;

			virtual hlBool Write(hlChar iChar) = 0;
			virtual hlUInt Write(hlVoid *lpData, hlUInt uiBytes) = 0;
		};
		...
		public:
			...
			virtual hlUInt GetStreamSize() const;
			virtual hlUInt GetStreamPointer() const;

			virtual hlUInt Seek(hlLong iOffset, HLSeekMode eSeekMode);

			virtual hlBool Read(hlChar &cChar);
			virtual hlUInt Read(hlVoid *lpData, hlUInt uiBytes);

			virtual hlBool Write(hlChar iChar);
			virtual hlUInt Write(hlVoid *lpData, hlUInt uiBytes);
		};
		...
		private:
			...
			hlUInt uiBlockEntryOffset;
			hlUInt uiDataBlockIndex;
			hlUInt uiDataBlockOffset;
		...
		private:
			...
			hlUInt uiMappingOffset;
			hlUInt uiMappingSize;
			hlUInt uiViewSize;
		...
		public:
			CMappingStream(Mapping::CMapping &Mapping, hlUInt uiMappingOffset, hlUInt uiMappingSize, hlUInt uiViewSize = HL_DEFAULT_VIEW_SIZE);
			~CMappingStream();
	...
	namespace Mapping
	{
		...
		private:
			...
			hlUInt uiOffset;
			hlUInt uiLength;
			hlUInt uiAllocationOffset;
			hlUInt uiAllocationLength;

		public:
			CView(CMapping *pMapping, hlVoid *lpView, hlUInt uiAllocationOffset, hlUInt uiAllocationLength, hlUInt uiOffset = 0, hlUInt uiLength = 0);
			~CView();

			CMapping *GetMapping() const;

			const hlVoid *GetView() const;
			hlUInt GetOffset() const;
			hlUInt GetLength() const;
			const hlVoid *GetAllocationView() const;
			hlUInt GetAllocationOffset() const;
			hlUInt GetAllocationLength() const;
		};
		...
		public:
			...
			hlUInt GetTotalMemoryAllocated() const;
			hlUInt GetTotalMemoryUsed() const;

			hlBool Open(hlUInt uiMode);
			hlVoid Close();

			virtual hlUInt GetMappingSize() const = 0;

			hlBool Map(CView *&pView, hlUInt uiOffset, hlUInt uiLength);
		...
		private:
			...
			virtual hlBool MapInternal(CView *&pView, hlUInt uiOffset, hlUInt uiLength) = 0;
		...
		public:
			...
			virtual hlUInt GetMappingSize() const;

		private:
			virtual hlBool OpenInternal(hlUInt uiMode);
			virtual hlVoid CloseInternal();

			virtual hlBool MapInternal(CView *&pView, hlUInt uiOffset, hlUInt uiLength);
			virtual hlVoid UnmapInternal(CView &View);
		};
		...
		public:
			CMemoryMapping(hlVoid *lpData, hlUInt uiBufferSize);
	...
	public:
		...
		hlUInt GetItemAttributeCount(const CDirectoryItem *pItem) const;
		...
		hlBool CreateStream(const CDirectoryFile *pFile, Streams::IStream *&pStream);
		hlVoid ReleaseStream(Streams::IStream *pStream);
	...
	protected:
		...
		virtual hlUInt GetItemAttributeCountInternal(const CDirectoryItem *pItem) const;
		...
		virtual hlBool CreateStreamInternal(const CDirectoryFile *pFile, Streams::IStream *&pStream) = 0;
		virtual hlVoid ReleaseStreamInternal(Streams::IStream &Stream);
	...
	protected:
		...
		virtual hlBool CreateStreamInternal(const CDirectoryFile *pFile, Streams::IStream *&pStream);
		virtual hlVoid ReleaseStreamInternal(Streams::IStream &Stream);
	...
	private:
		...
		struct GCFUnknownHeader
		{
			hlUInt uiDummy0;			// Always 0x00000001
			hlUInt uiDummy1;			// Always 0x00000000
		};

		struct GCFUnknownEntry
		{
			hlUInt uiDummy0;
		};
	...
	private:
		...
		GCFUnknownHeader *pUnknownHeader;
		GCFUnknownEntry *lpUnknownEntries;
	...
	private:
		hlBool GetLumpInfo(const CDirectoryFile &File, hlUInt &uiWidth, hlUInt &uiHeight, hlUInt &uiPaletteSize, hlUInt uiMipmap = 0);
		hlBool GetLumpInfo(const CDirectoryFile &File, hlUInt &uiWidth, hlUInt &uiHeight, hlUInt &uiPaletteSize, const hlByte *&lpPalette, const hlByte *&lpPixels, Mapping::CView *&pView, hlUInt uiMipmap = 0);
	};
