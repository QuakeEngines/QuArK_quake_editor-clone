...
HRESULT NVMipmapCallback(void *data, int miplevel, DWORD size, int width, int height, void *user_data)
{
	assert(user_data != 0);

	SNVCompressionUserData *UserData = static_cast<SNVCompressionUserData *>(user_data);

	// Set the image data of a VTFFile object for the specified face and frame.
	if(UserData->pVTFFile != 0)
	{
		assert(size == CVTFFile::ComputeImageSize(width, height, 1, UserData->ImageFormat));

		if(UserData->ImageFormat == UserData->pVTFFile->GetFormat())
		{
			UserData->pVTFFile->SetData(UserData->uiFrame, UserData->uiFace, UserData->uiSlice, miplevel, (vlByte *)data);
		}
		else
		{
			...
			CVTFFile::ConvertFromRGBA8888((vlByte *)data, UserData->pVTFFile->GetData(UserData->uiFrame, UserData->uiFace, UserData->uiSlice, miplevel), width, height, UserData->pVTFFile->GetFormat());
		}
	}
	...
	else if(UserData->lpData != 0)
	{
		...
		memcpy(UserData->lpData, data, CVTFFile::ComputeImageSize(width, height, 1, UserData->ImageFormat));
	}
	else
	{
		return E_FAIL;
...
HRESULT NVFileWriteCallback(size_t count, void *buffer, void * userData)
{
	return S_OK;
}

vlBool nvDXTCompressWrapper(vlByte *lpImageDataRGBA, vlUInt uiWidth, vlUInt uiHeight, CompressionOptions *Options, MIPcallback NVMipmapCallback)
{
	// nvDXTcompressRGBA() seems unstable.  Maybe it is a problem with the options?
	try
	{
		if(nvDXTcompressRGBA(lpImageDataRGBA, uiWidth, uiHeight, uiWidth * 4, Options, 4, NVMipmapCallback, NULL, NULL) == S_OK)
		{
			return vlTrue;
		}
		else
		{
			LastError.Set("nvDXTcompressRGBA() failed.");
	...
	{
		LastError.Set("nvDXTcompressRGBA() crashed.");
...
CVTFFile::CVTFFile(const CVTFFile &VTFFile, VTFImageFormat ImageFormat)
{
	...
	if(VTFFile.IsLoaded())
	{
		...
		{
			this->Header->Flags |= TEXTUREFLAGS_NOCOMPRESS;
		}
		else
		{
			this->Header->Flags &= ~TEXTUREFLAGS_NOCOMPRESS;
...
vlBool CVTFFile::Create(vlUInt uiWidth, vlUInt uiHeight, vlUInt uiFrames, vlUInt uiFaces, vlUInt uiSlices, VTFImageFormat ImageFormat, vlBool bThumbnail, vlBool bMipmaps, vlBool bNullImageData)
{
	...
	if(uiWidth == 0 || (uiWidth & (uiWidth - 1)) != 0 || uiWidth > 0xffff)
	{
		LastError.Set("Invalid image width.  Width must be nonzero and a power of two.");
		return vlFalse;
	}

	// Check if height is valid (power of 2 and fits in a short).
	if(uiHeight == 0 || (uiHeight & (uiHeight - 1)) != 0 || uiHeight > 0xffff)
	{
		LastError.Set("Invalid image height.  Height must be nonzero and a power of two.");
		return vlFalse;
	}

	// Check if height is valid (power of 2 and fits in a short).
	if(uiSlices == 0 || (uiSlices & (uiSlices - 1)) != 0 || uiSlices > 0xffff)
	{
		LastError.Set("Invalid image depth.  Depth must be nonzero and a power of two.");
		return vlFalse;
	}
	...
	{
		LastError.Set("Invalid image frame count.");
		return vlFalse;
	}

	if(uiFaces != 1 && uiFaces != 6 && uiFaces != 7)
	{
		LastError.Set("Invalid image face count.");
		return vlFalse;
	}
	...
	this->Header->HeaderSize = sizeof(SVTFHeader);
	this->Header->Width = (vlShort)uiWidth;
	this->Header->Height = (vlShort)uiHeight;
	this->Header->Flags = (this->GetImageFormatInfo(ImageFormat).bIsCompressed ? 0 : TEXTUREFLAGS_NOCOMPRESS)
...
vlBool CVTFFile::Create(vlUInt uiWidth, vlUInt uiHeight, vlUInt uiFrames, vlUInt uiFaces, vlUInt uiSlices, vlByte **lpImageDataRGBA8888, const SVTFCreateOptions &VTFCreateOptions)
{
	...
	if(VTFCreateOptions.uiVersion[0] != 7 || (VTFCreateOptions.uiVersion[1] != 0 && VTFCreateOptions.uiVersion[1] != 1 && VTFCreateOptions.uiVersion[1] != 2))
	{
		LastError.Set("File version does not match 7.0 or 7.1 or 7.2.");
		return vlFalse;
	}
	...
	try
	{
		if(VTFCreateOptions.bResize)
		{
			...
			case RESIZE_SMALLEST_POWER2:
				...
				else
				{
					...
					for(vlUInt i = 0, uiMask = 0xffffffff; i < sizeof(vlUInt) * 8; i++, uiMask >>= 1)
					{
						if(uiWidth & ~uiMask)
						{
							uiNewWidth = uiWidth & ~uiMask;
							break;
						}
					}

					if(VTFCreateOptions.ResizeMethod == RESIZE_BIGGEST_POWER2)
					{
						uiNewWidth <<= 1;
					}
					else if(VTFCreateOptions.ResizeMethod == RESIZE_NEAREST_POWER2)
					{
						if((uiNewWidth << 1) - uiWidth <= uiWidth - uiNewWidth)
						{
							uiNewWidth <<= 1;
						}
					}
				...
				else
				{
					...
					{
						if((uiNewHeight << 1) - uiHeight <= uiHeight - uiNewHeight)
						{
							uiNewHeight <<= 1;
						}
					}
		...
		if(VTFCreateOptions.bNormalMap && uiFaces == 1)
		{
			...
			{
				this->Header->Flags |= TEXTUREFLAGS_PREMULTCOLORBYONEOVERMIPLEVEL | TEXTUREFLAGS_NORMALTODUDV;
			}
			else
			{
			...
			for(vlUInt i = 0; i < uiFrames; i++)
			{
				for(vlUInt j = 0; j < uiFaces; j++)
				{
					for(vlUInt k = 0; k < uiSlices; k++)
					{
						...
						Options.mipFilterType = (MipFilterTypes)VTFCreateOptions.MipmapFilter;

						// Setup sharpen filter.
						if(VTFCreateOptions.MipmapSharpenFilter != SHARPEN_FILTER_NONE)
						{
							Options.sharpenFilterType = (SharpenFilterTypes)VTFCreateOptions.MipmapSharpenFilter;
							Options.sharpening_passes_per_mip_level[0] = 0;
							for(vlUInt l = 1; l < MAX_MIP_MAPS; l++)
							{
								Options.sharpening_passes_per_mip_level[l] = 1;
							}
							Options.unsharp_data.radius = sUnsharpenRadius;
							Options.unsharp_data.amount = sUnsharpenAmount;
							Options.unsharp_data.threshold = sUnsharpenThreshold;
							Options.xsharp_data.strength = sXSharpenStrength;
							Options.xsharp_data.threshold = sXSharpenThreshold;
						}

						// Set the format.
						Options.quality = (QualitySetting)(DXT_QUALITY_BASE + uiDXTQuality);
						...
						case IMAGE_FORMAT_RGBA8888:
							Options.bSwapRGB = true;	// Our input is RGB but the output is still BGR unless we set this.
						...
						if(!nvDXTCompressWrapper(lpImageDataRGBA8888[i + j + k], this->Header->Width, this->Header->Height, &Options, NVMipmapCallback))
						{
							throw 0;
						}
					}
		...
		for(vlUInt uiFlag = 0x00000001; uiFlag <= TEXTUREFLAGS_LAST; uiFlag <<= 1)
		{
...
vlBool CVTFFile::Load(IO::Readers::IReader *Reader, vlBool bHeaderOnly)
{
	...
	try
	{
		...
		if(FileHeader.Version[0] != 7 || (FileHeader.Version[1] != 0 && FileHeader.Version[1] != 1 && FileHeader.Version[1] != 2))
		{
			LastError.Set("File version does not match 7.0 or 7.1 or 7.2.");
			throw 0;
		}
		...
		if(this->Header->Version[0] <= 7 && this->Header->Version[1] <= 1 || this->Header->Depth < 1)
		{
			// set depth if version is 7.1 or lower
			this->Header->Depth = 1;
		}
		...
		if(this->Header->HeaderSize + this->uiThumbnailBufferSize + this->uiImageBufferSize > uiFileSize)
		{
			LastError.Set("File is corrupt; file to small for it's image data.");
			throw 0;
		}

		Reader->Seek(this->Header->HeaderSize, FILE_BEGIN);
...
vlBool CVTFFile::Save(IO::Writers::IWriter *Writer) const
{
	...
	try
	{
		...
		case 7:
			...
			{
			case 0:
				uiHeaderDataSize = sizeof(SVTFHeader_70);
				this->Header->HeaderSize = sizeof(SVTFHeader_70_A);
				break;
			case 1:
				uiHeaderDataSize = sizeof(SVTFHeader_71);
				this->Header->HeaderSize = sizeof(SVTFHeader_71_A);
				break;
			case 2:
				uiHeaderDataSize = sizeof(SVTFHeader_72);
		...
		// Write the header.
		if(Writer->Write(this->Header, uiHeaderDataSize) != uiHeaderDataSize)
		{
			throw 0;
		}

		// write padding bytes after we write the header (for 16 byte alignment)
		// if we don't do this we could end up with data in the header we don't
		// want being written, it shouldn't cause problems, but this keeps the
		// input and output file identical if no changes were made
		uiHeaderDataSize = this->Header->HeaderSize - uiHeaderDataSize;
		while(uiHeaderDataSize)
		{
			Writer->Write(0);
			uiHeaderDataSize--;
		}
...
vlUInt CVTFFile::GetSize() const
{
	...
	return this->Header->HeaderSize + this->uiThumbnailBufferSize + this->uiImageBufferSize;
}
...
vlVoid CVTFFile::SetFlag(VTFImageFlag ImageFlag, vlBool bState)
{
	...
	if(ImageFlag == TEXTUREFLAGS_NOCOMPRESS || ImageFlag == TEXTUREFLAGS_ONEBITALPHA || ImageFlag == TEXTUREFLAGS_EIGHTBITALPHA || ImageFlag == TEXTUREFLAGS_ENVMAP)
	{
		return;
	}
...
vlBool CVTFFile::GenerateMipmaps(vlUInt uiFace, vlUInt uiFrame, VTFMipmapFilter MipmapFilter, VTFSharpenFilter SharpenFilter)
{
	...
	Options.mipFilterType = (MipFilterTypes)MipmapFilter;

	// Setup sharpen filter.
	if(SharpenFilter != MAX_MIP_MAPS)
	{
		Options.sharpenFilterType = (SharpenFilterTypes)SharpenFilter;
		Options.sharpening_passes_per_mip_level[0] = 0;
	...
	if(!nvDXTCompressWrapper(lpImageData, this->Header->Width, this->Header->Height, &Options, NVMipmapCallback))
	{
		delete []lpImageData;
...
static SVTFImageFormatInfo VTFImageFormatInfo[] =
{
	...
	{ "I32F",				 32,  4, 32,  0,  0,  0, vlFalse,  vlTrue },		// IMAGE_FORMAT_I32F
...
vlBool CVTFFile::CompressDXTn(vlByte *lpSource, vlByte *lpDest, vlUInt uiWidth, vlUInt uiHeight, VTFImageFormat DestFormat)
{
	...
	return nvDXTCompressWrapper(lpSource, uiWidth, uiHeight, &Options, NVMipmapCallback);
#else
...
static SVTFImageConvertInfo VTFImageConvertInfo[] =
{
	...
	{ 	 32,  4, 32,  0,  0,  0,	 0,	-1,	-1,	-1, vlFalse, vlFalse,	NULL,	NULL,		IMAGE_FORMAT_I32F},
...
vlBool ConvertTemplated(vlByte *lpSource, vlByte *lpDest, vlUInt uiWidth, vlUInt uiHeight, const SVTFImageConvertInfo& SourceInfo, const SVTFImageConvertInfo& DestInfo)
{
	...
	for(; lpSource < lpSourceEnd; lpSource += SourceInfo.uiBytesPerPixel, lpDest += DestInfo.uiBytesPerPixel)
	{
		...
		T Source = 0;
		for(vlUInt i = 0; i < SourceInfo.uiBytesPerPixel; i++)
		{
		...
		for(vlUInt i = 0; i < DestInfo.uiBytesPerPixel; i++)
		{
...
vlBool CVTFFile::ConvertToNormalMap(vlByte *lpSourceRGBA8888, vlByte *lpDestRGBA8888, vlUInt uiWidth, vlUInt uiHeight, VTFKernelFilter KernelFilter, VTFHeightConversionMethod HeightConversionMethod, VTFNormalAlphaResult NormalAlphaResult, vlByte bMinimumZ, vlSingle sScale, vlBool bWrap, vlBool bInvertX, vlBool bInvertY, vlBool bInvertZ)
{
	...
	Options.normalMap.filterKernel = (NormalMapFilters)(KERNEL_FILTER_BASE + (vlInt)KernelFilter);
	Options.normalMap.heightConversionMethod = (HeightConversionMethods)(HEIGHT_CONVERSION_METHOD_BASE + (vlInt)HeightConversionMethod);
	Options.normalMap.alphaResult = (AlphaResult)(NORMAL_ALPHA_RESULT_BASE + (vlInt)NormalAlphaResult);
...
//	Options.normalMap.bSignedOutput = KernelFilter == KERNEL_FILTER_DUDV;	// Normal maps seem to be signed in Source, DuDv maps don't.
	...
	return nvDXTCompressWrapper(lpSourceRGBA8888, uiWidth, uiHeight, &Options, NVMipmapCallback);
#else
...
vlBool CVTFFile::Resize(vlByte *lpSourceRGBA8888, vlByte *lpDestRGBA8888, vlUInt uiSourceWidth, vlUInt uiSourceHeight, vlUInt uiDestWidth, vlUInt uiDestHeight, VTFMipmapFilter ResizeFilter, VTFSharpenFilter SharpenFilter)
{
	...
	// Set resize filter.
	Options.rescaleImageFilter = (MipFilterTypes)ResizeFilter;
	...
	return nvDXTCompressWrapper(lpSourceRGBA8888, uiSourceWidth, uiSourceHeight, &Options, NVMipmapCallback);
#else
