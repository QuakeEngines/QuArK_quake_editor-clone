...
int main(int argc, char* argv[])
{
	...
	vlBool bHelp = vlFalse;				// Display help only.
		...
		for(i = 1; i < argc; i++)
		{
			...
			else if(stricmp(argv[i], "-format") == 0)
			{
				if(i + 1 < argc)
				{
					...
					{
						CreateOptions.ImageFormat = ImageFormat;
					}
					else
					{
	...
	{
		PrintUsage("-file and -folder not specified.");
		return 2;
	}
...
void PrintUsage(const vlChar *lpError, ...)
{
	...
	Print(" -format <string>         (Ouput format to use.)\n");
	...
	Print("vtfcmd.exe -file \"C:\\texture.bmp\" -format \"bgr888\" -normal -postfix \"_normal\"\n");
...
void ProcessFile(vlChar *lpInputFile)
{
	...
	vlChar lpTGAFile[512];			// Holds output .tga file name.
	...
	vlByte *lpImageData;			// TGA data.
	VTFImageFormat DestFormat;		// TGA format.
	...
	else
	{
		...
		if(!ilTexImage(vlImageGetWidth(), vlImageGetHeight(), 1, DestFormat == IMAGE_FORMAT_RGBA8888 ? 4 : 3, DestFormat == IMAGE_FORMAT_RGBA8888 ? IL_RGBA : IL_RGB, IL_UNSIGNED_BYTE, lpImageData))
		{
			...
			Print("  Error creating tga file.\n\n");
			return;
		}

		free(lpImageData);

		CreateOutputPath(lpTGAFile, lpInputFile, ".tga");

		// Write tga file.
		Print("  Writing %s...\n", lpTGAFile);
		if(!ilSaveImage(lpTGAFile))
		{
			Print(" Error creating tga file.\n\n");
			return;
		}
