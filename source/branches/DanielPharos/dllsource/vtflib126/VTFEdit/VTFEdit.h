...
namespace VTFEdit
{
	...
	private:
		...
		private: void SetVTFFile(VTFLib::CVTFFile *VTFFile)
		{
			...
			this->lstFlags->Items->Add(S"Unused", uiFlags & TEXTUREFLAGS_UNUSED);
			this->lstFlags->Items->Add(S"Unfilterable OK", uiFlags & TEXTUREFLAGS_UNFILTERABLE_OK);
			this->lstFlags->Items->Add(S"Clamp All", uiFlags & TEXTUREFLAGS_BORDER);
			this->lstFlags->Items->Add(S"SpecVar Red", uiFlags & TEXTUREFLAGS_SPECVAR_RED);
			this->lstFlags->Items->Add(S"SpecVar Alpha", uiFlags & TEXTUREFLAGS_SPECVAR_ALPHA);
